...
RAG_SYSTEM_PROMPT = """You are a precise, grounded assistant. Answer questions ONLY using the provided context.

Rules:
- If the context contains the answer, answer clearly and cite the source.
- If the context is insufficient, say exactly: "INSUFFICIENT_CONTEXT"
...
def rag_node(state: AgentState, llm: LLMClientBase, retriever: HybridRetriever) -> AgentState:
    ...
    # ── Direct answer path (router said no retrieval needed) ────────────────
    ...
    # On retry, we could expand query here (Phase 4 enhancement: query expansion)
